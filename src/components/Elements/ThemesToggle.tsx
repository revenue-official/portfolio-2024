...
export default function ThemesToggle({ className = '', children }: SideLinkProps) {
  ...
  useEffect(() => {
    if (resolvedTheme === 'dark') {
      setTheme('dark');
      setIcon(<Moon className="h-8 w-8 flex-shrink-0 p-1 md:ml-2 md:mr-4 md:h-5 md:w-5 md:p-0" />);
      setTitle('Dark Mode');
    } else {
      setTheme('light');
      setIcon(<Sun className="h-8 w-8 flex-shrink-0 p-1 md:ml-2 md:mr-4 md:h-5 md:w-5 md:p-0" />);
      setTitle('Light Mode');
    }
